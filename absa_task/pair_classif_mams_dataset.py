...
def run_onestep_ft():
    ...
    utils.create_fasttext_input_files("mams_categories", "valid", "ft_onestep_pair_classif", my_data.X_val_full,
                                      my_data.y_val_full)
    utils.create_fasttext_input_files("mams_categories", "train", "ft_onestep_pair_classif", my_data.X_train_full,
...
def run_twostep_ft():
    ...
    utils.create_fasttext_input_files("mams_categories", "valid", "ft_twotep_pair_classif_categ",
                                      my_data.X_val_full,
                                      y_val_categories)
    utils.create_fasttext_input_files("mams_categories", "train", "ft_twotep_pair_classif_categ",
                                      my_data.X_train_full,
                                      y_train_categories)

    y_pred_categories, y_pred_probs = ft_classification_multilabel("mams_categories",
                                                                   my_data.X_test_full,
    ...
    utils.create_fasttext_input_files("mams_categories", "valid", "ft_twotep_pair_classif_polar", X_val_polarities,
                                      y_val_polarities)
    utils.create_fasttext_input_files("mams_categories", "train", "ft_twotep_pair_classif_polar", X_train_polarities,
                                      y_train_polarities)
    X_test_polarities = utils.create_as_input_x(my_data.X_test_full, y_pred_categories)

    y_pred_polarities, y_pred_probs = ft_classification_onelabel("mams_categories",
                                                                 X_test_polarities,
...
def run_lstm():
    mams_dataset = MamsDataset.Mams()

    y_train = utils.convert_to_onehot(mams_dataset.y_train_full, mams_dataset.labels_mapping_dict,
                                      mams_dataset.NUM_LABELS)
    y_test = utils.convert_to_onehot(mams_dataset.y_test_full, mams_dataset.labels_mapping_dict,
                                     mams_dataset.NUM_LABELS)
    y_val = utils.convert_to_onehot(mams_dataset.y_val_full, mams_dataset.labels_mapping_dict, mams_dataset.NUM_LABELS)

    X_train, vocab_size = tokenize_sentences(mams_dataset.X_train_full)
    X_test, _ = tokenize_sentences(mams_dataset.X_test_full)
    X_val, _ = tokenize_sentences(mams_dataset.X_val_full)

    model = train_lstm_multilabel(vocab_size, mams_dataset.NUM_LABELS, X_train, y_train, X_val, y_val)
