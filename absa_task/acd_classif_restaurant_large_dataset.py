...
def run_onestep_ft():
    ...
    utils.create_fasttext_input_files("restaurant_large", "valid", "ft_acd", my_data.X_val_full,
                                      my_data.y_val_full)
    utils.create_fasttext_input_files("restaurant_large", "train", "ft_acd", my_data.X_train_full,
...
def run_lstm():
    ...
    evaluate_lstm(model, X_test, y_test, 0.5, my_data.categories_mapping_dict)
...
def run_use():
    ...
    print(classification_report(y_test_converted, y_preds_onehot, target_names=my_data.categories_mapping_dict, zero_division=0))
